...
async run(message, args, client) {
    ...
    let confirmation = new Discord.MessageEmbed()
        ...
        .setDescription(`Are you sure, you want to set ${channel} as the boost Channel?`)
        .setColor("BLURPLE")

    let cancelled = new Discord.MessageEmbed()
        .setTitle(`Cancelled`)
        .setDescription(`Cancelled setting ${channel} as the boost Channel.`)
        .setColor("RED")

    let confirmed = new Discord.MessageEmbed()
        .setTitle(`Confirmed`)
        .setDescription(`${channel} has been set as the boost Channel.`)
        .setColor("GREEN")
    ...
    if (!data) {
        try {
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.customId === 'yes') {
                    ...
                    await data1.updateOne({ BoostChannel: channel })
        ...
        collector.on('collect', async (i) => {
            i.deferUpdate()
            if (i.customId === 'yes') {
                await data.updateOne({ BoostChannel: channel })
                await m.edit({ embeds: [confirmed], components: [disabledrow] })
                return;
            }
            else if (i.customId === 'no') {
