...
module.exports = new Command({
    ...
    async run(message, args, client) {
      ...
      .setDescription("Please mention the role you wish set the verify role as!")
      if (!role) return message.reply({embeds: [noChannel]})
      const data = await CaptchaSchema.findOne({ guildId: message.guild.id }) // find the channel and store it in the DB

      let confirmation = new Discord.MessageEmbed()
          .setTitle(`Please Confirm`)
          .setDescription(`Are you sure, you want to set ${role} as the verify role?`)
          .setColor("BLURPLE")

      let cancelled = new Discord.MessageEmbed()
          .setTitle(`Cancelled`)
          .setDescription(`Cancelled setting ${role} as the verified role`)
          .setColor("RED")

      let confirmed = new Discord.MessageEmbed()
          .setTitle(`Confirmed`)
          .setDescription(`${role} has been set as the verified role.`)
          .setColor("GREEN")
      ...
      if (!data) {
          ...
          collector.on('collect', async (i) => {
              i.deferUpdate()
              if (i.customId === 'yes') {
                  await data.updateOne({ CaptchaSchema: role.id })
                  await m.edit({ embeds: [confirmed], components: [disabledrow] })
                  return;
              }
              else if (i.customId === 'no') {
