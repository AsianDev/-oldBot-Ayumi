...
async run(message, args, client) {
    ...
    let confirmation = new Discord.MessageEmbed()
        ...
        .setDescription(`Are you sure, you want to set ${channel} as the Goodbye Channel?`)
        .setColor("BLURPLE")

    let cancelled = new Discord.MessageEmbed()
        .setTitle(`Cancelled`)
        .setDescription(`Cancelled setting ${channel} as the Goodbye Channel.`)
        .setColor("RED")
    ...
    if (!data) {
        try {
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.customId === 'yes') {
                    ...
                    await data1.updateOne({ leaveChannel: channel })
        ...
        collector.on('collect', async (i) => {
            i.deferUpdate()
            if (i.customId === 'yes') {
                await data.updateOne({ leaveChannel: channel })
                await m.edit({ embeds: [confirmed], components: [disabledrow] })
                return;
            }
            else if (i.customId === 'no') {
