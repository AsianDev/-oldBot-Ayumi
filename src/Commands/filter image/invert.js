...
module.exports = new Command({
    ...
    async run(message, args, client) {
         ...
         try {
          ...
          const buffer = await AmeAPI.generate("invert", { url: user.displayAvatarURL({ format: "png", size: 512 }) });
           const attachment = new Discord.MessageAttachment(buffer, "invert.png");

           m.edit({ content: "Generated <:Iki_mochaHeart:939483315952578610>", files: [attachment], name: "invert.png", allowedMentions: {repliedUser: false}});
         } catch (err) {
