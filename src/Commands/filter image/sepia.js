...
module.exports = new Command({
    ...
    async run(message, args, client) {
         ...
         try {
           ...
           const buffer = await AmeAPI.generate("sepia", { url: user.displayAvatarURL({ format: "png", size: 512 }) });
           const attachment = new Discord.MessageAttachment(buffer, "sepia.png");

               m.edit({ content: "Generated <a:Kao_Thumbsup:936968227274256424>", files: [attachment], name: "sepia.png", allowedMentions: {repliedUser: false}});
         } catch (err) {
