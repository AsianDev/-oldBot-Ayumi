...
module.exports = new Command({
    ...
    async run(message, args, client) {
         ...
         try {
           ...
           const buffer = await AmeAPI.generate("circle", { url: user.displayAvatarURL({ format: "png", size: 512 }) });
           const attachment = new Discord.MessageAttachment(buffer, "circle.png");

               m.edit({ content: "Generated <a:Kao_Thumbsup:936968227274256424>", files: [attachment], name: "circle.png", allowedMentions: {repliedUser: false}});
         } catch (err) {
