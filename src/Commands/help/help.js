...
const newRow = new Discord.MessageActionRow()        
         ...
         // image gen
          ...
          const ImageRow = new Discord.MessageActionRow()        
              ...
              const Modrow = new Discord.MessageActionRow()        
                ...
                const setupRow = new Discord.MessageActionRow()        
                  .addComponents(
                    ...
                    .addField("For more help with backup commands:", "Run ``Ayu help backup``")
                    .setTimestamp()
                    .setColor("BLUE")
                    .setThumbnail(message.member.user.avatarURL({ dynamic: true }))
                    .setFooter({ text:` > Ayumi • ${message.channel.name}`})
            ...
            .addComponents(
              ...
              .setThumbnail(message.member.user.avatarURL({ dynamic: true }))
              .setFooter({ text:` > Ayumi • ${message.channel.name}`})
              const wallpaperRow = new Discord.MessageActionRow()
              .addComponents(
                new Discord.MessageButton()
                .setURL("https://top.gg/servers/873143392488525834")
                .setLabel("Vote for Ikigai")
  ...
  else if((args[1] === 'dev'|| args[1] === 'Dev' || args[1] === 'Develepor' || args[1] === 'develepor') && message.member.roles.cache.has('923825269331070976')){  message.reply({ embeds: [DevEmbed], components: [devrow], allowedMentions: {repliedUser: false}}) }
  else if(args[1] === 'wallpaper' || args[1] === 'Wallpaper' || args[1] === 'background' || args[1] === 'bg' || args[1] === 'Background' || args[1] === 'backgrounds' || args[1] === 'wallpapers'){  message.reply({ embeds: [WallPaperEmbed], components: [wallpaperRow], allowedMentions: { repliedUser: false }}) }
